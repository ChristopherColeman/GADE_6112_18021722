...
namespace Assignment_1
{
    class MeleeUnit : Unit // Tells the system to inherit the abstract classes found in the Unit class
    {
        ...
        {
            if (DistanceTo(ref Attacker) == range) //error in previous code, no ref keyword supplied
            {
                return true;
            }
            else
        ...
        public override Unit Closest(ref Unit[] map) //using the ref keyword here yet the "u" still errors
        {
            Unit closest = this;
            ...
            foreach (Unit u in map)
            {
                ...
                {   // still not sure of the "u" error here, no problem with it in the RangedUnit
